...
public class CapacitySchedulerServlet extends HttpServlet {
  ...
  private void showQueues(PrintWriter out) throws IOException {
    synchronized (scheduler) {
      ...
      out.print("<tr><th>Queue</th><th>Running Jobs</th>"
          + "<th>Pending Jobs</th>" + "<th>Capacity Percentage</th>"
          + "<th>Map Task Capacity</th><th>Map Task Used Capacity</th>"
          + "<th>Running Maps</th><th>Reduce Task Capacity</th>"
          + "<th>Reduce Task Used Capacity</th><th>Running Reduces </tr>\n");
      JobQueuesManager queuesManager = scheduler.getJobQueuesManager();
      for (QueueSchedulingContext qsc : scheduler.getQueueInfoMap().values()) {
        String name = qsc.getQueueName();
        out.print("<tr>\n");
        out.printf(
        ...
        out.printf("<td %.1f%%</td>\n", qsc.getCapacityPercent());

        int mapCapacity = qsc.getMapCapacity();
        int mapSlotsOccupied = qsc.getMapTSC().getNumSlotsOccupied();
        float occupiedSlotsAsPercent = mapCapacity != 0 ? ((float) mapSlotsOccupied * 100 / mapCapacity)
            : 0;
        out.printf("<td>%s</td>\n", mapCapacity);
        out.printf("<td>%s (%.1f%% of Capacity)</td>\n", mapSlotsOccupied,
            occupiedSlotsAsPercent);
        out.printf("<td>%s</td>\n", qsc.getMapTSC().getNumRunningTasks());
        int reduceCapacity = qsc.getReduceCapacity();
        int reduceSlotsOccupied = qsc.getReduceTSC().getNumSlotsOccupied();
        float redOccupiedSlotsAsPercent = reduceCapacity != 0 ? ((float) reduceSlotsOccupied * 100 / reduceCapacity)
            : 0;
        out.printf("<td>%s</td>\n", reduceCapacity);
        out.printf("<td>%s (%.1f%% of Capacity)</td>\n", reduceSlotsOccupied,
            redOccupiedSlotsAsPercent);
        out.printf("<td>%s</td>\n", qsc.getReduceTSC().getNumRunningTasks());
      }
      out.print("</table>\n");
    }
  }
